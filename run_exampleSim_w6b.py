...
burnin_id = 'da4b68ad-f017-ed11-a9fb-b88303911bc1'
...
# Add intervention
def itn_intervention(cb, coverage_level=0.8):
    add_ITN(cb,
...
add_summary_report(cb, start=1, interval=365,
                   age_bins=[0.25, 2, 5, 10, 15, 20, 100],
...
expt_name = f'{user}_FE_2022_pick_up{pull_year}'
...
builder = ModBuilder.from_list([[ModFn(case_management, cm_cov_U5),
                                 ...
                                 ModFn(DTKConfigBuilder.set_param, 'x_Temporary_Larval_Habitat', row['x_Temporary_Larval_Habitat']),
                                 ]
                                for cm_cov_U5 in [0.6, 0.7]
                                for itn_cov in [0, 0.5]
                                #for hab_scale in np.logspace(-2, np.log10(30), 3)
